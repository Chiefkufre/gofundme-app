...
class CampaignForm(FlaskForm):
    ...
    describe_campaign = StringField(
        "Craft a compelling story about your campaign",
        validators=[
            DataRequired(),
            Length(min=5, message="must be more than eight characters"),
        ],
    )
    campaign_owner = SelectField(
        "Who are you raising the fund for?",
        validators=[DataRequired()],
        choices=[
            (
                "Yourself-Fund will be credited into your account",
                "Yourself-Fund will be credited into your account",
            ),
            (
                "Someone else-You will invite a beneficiary to receive fund or distribute it yourself",
                "Someone else-You will invite a beneficiary to receive fund or distribute it yourself",
            ),
        ],
    )
...
class ContactForm(FlaskForm):
    ...
    subject = StringField(
        "Subject", validators=[DataRequired(), Length(min=10, max=100)]
    )
